...
fn collect_dom_errors(
    ...
    if let Err(errors) = dom.validate() {
        for error in errors {
            match &error {
                olpat::dom::Error::ConflictingKeys { key, other } => {
                    let range = doc
                        .mapper
                ...
                olpat::dom::Error::ExpectedArrayOfTables {
                    not_array_of_tables,
                    required_by,
                } => {
                    let range = doc
                ...
                olpat::dom::Error::InvalidEscapeSequence { string: _ }
                | olpat::dom::Error::Query(_) => {}
                olpat::dom::Error::UnexpectedSyntax { syntax } => {
