use olpat_common::environment::{native::NativeEnvironment, Environment};
...
impl<E: Environment> Taplo<E> {
    pub async fn execute_lsp(&mut self, cmd: LspCommand) -> Result<(), anyhow::Error> {
        ...
        let server = olpat_lsp::create_server();
        let world = olpat_lsp::create_world(NativeEnvironment::new());
