...
pub async fn run_cli(env: JsValue, args: JsValue) -> Result<(), JsError> {
    ...
    use olpat_common::{environment::Environment, log::setup_stderr_logging};
    ...
    match Taplo::new(env.clone())
        .execute(cli)
        .instrument(tracing::info_span!("olpat"))
        .await
    {
        Ok(_) => Ok(()),
        Err(error) => {
...
pub fn create_lsp(env: JsValue, lsp_interface: JsValue) -> lsp::TaploWasmLsp {
    ...
    lsp::TaploWasmLsp {
        server: olpat_lsp::create_server(),
        world: olpat_lsp::create_world(env),
